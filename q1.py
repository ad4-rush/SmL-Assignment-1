...
def qda_predict0(sample):
    ...
    for i in [2,3,5,6,8]:
        ...
        if log_likelihood>predictions:
            ii=i
            predictions = log_likelihood
    return(predictions,ii)
def qda_predict1(sample):
    predictions = -10000
    ii=0
    for i in [4,7,9]:
        mean_vector = mean_vectors[i]
...
def qda_predict(sample):
    mean_vector = mean_vectors[8]
    cov_matrix = cov_matrices[8]
    log_det = log_dets[8]
    # QDA discriminant function with log determinant
    log_likelihood0 = -0.5 * log_det \
                        - 0.5 * np.dot(np.dot((sample - mean_vector), np.linalg.inv(cov_matrix)), (sample - mean_vector).T)
    mean_vector = mean_vectors[1]
    cov_matrix = cov_matrices[1]
    log_det = log_dets[1]
    # QDA discriminant function with log determinant
    log_likelihood1 = -0.5 * log_det \
                        - 0.5 * np.dot(np.dot((sample - mean_vector), np.linalg.inv(cov_matrix)), (sample - mean_vector).T)
    if(log_likelihood0>log_likelihood1):
        temp = qda_predict0(sample)
        if temp[0]>log_likelihood0:
            return 0
        else:
            return temp[1]
    else:
        temp = qda_predict1(sample)
        if temp[0]>log_likelihood1:
            return 1
        else:
            return temp[1]
